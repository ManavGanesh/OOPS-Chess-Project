...
public class TilePanel extends JPanel {
    ...
    public Board chessBoard; // Reference for cleanup - made public for BoardPanel access
    private javax.swing.Timer dragTimeoutTimer; // Safety timer to prevent stuck drags
    
    // Move preview system fields
    private boolean showMoveIndicator = false;
    private boolean isSafeMove = true; // Green if safe, red if piece will be under attack
    private static final int MOVE_INDICATOR_DIAMETER = 20; // Circle diameter
    ...
    private void handleMouseClick(MouseEvent e, Board chessBoard) {
        ...
        boolean isPaired = isAIMode || (client != null && client.isPaired);
        ...
        // ENHANCED VALIDATION: Add null checks and better error handling
        if (playerTeam == null) {
            System.out.println("DEBUG: Player team is null - cannot validate turn");
            return;
        }
    ...
    private void handleMouseDragged(MouseEvent e, Board chessBoard) {
        if (!isDragging && draggedTile != null && dragStartPoint != null) {
            ...
            if (distance > 5) { // Minimum drag distance threshold
                ...
                // MOVE PREVIEW SYSTEM: Show legal moves when starting drag
                boardPanel.showLegalMovesForPiece(draggedTile);
    ...
    private void handleMouseReleased(MouseEvent e, Board chessBoard) {
        try {
            if (isDragging) {
                ...
                // MOVE PREVIEW SYSTEM: Clear move indicators before processing drop
                boardPanel.clearAllMoveIndicators();
    ...
    private void executeMoveLogic(Board chessBoard, boolean isAIMode, Coordinate targetCoord) {
        ...
        if (!isAIMode && client != null && client.isMovePending()) {
            System.out.println("DEBUG: Move already pending, blocking additional moves");
            return;
        }
        
        // CRITICAL FIX: In multiplayer mode, double-check that current player matches client team
        if (!isAIMode && client != null) {
            Team currentPlayerTeam = chessBoard.getCurrentPlayer().getTeam();
            Team clientTeam = client.getTeam();
            
            if (currentPlayerTeam != clientTeam) {
                System.out.println("DEBUG: Current player (" + currentPlayerTeam + ") doesn't match client team (" + clientTeam + ") - blocking move");
                return;
            }
        }
        ...
        if (!chessBoard.hasChosenTile()) {
            ...
            // MOVE PREVIEW SYSTEM: Show legal moves for the selected piece
            boardPanel.showLegalMovesForPiece(chessBoard.getTile(targetCoord));
        } else {
            ...
            if (MoveUtilities.isValidMove(chessBoard, destinationTile)) {
                ...
                if (isAIMode) {
                    ...
                    // MOVE PREVIEW SYSTEM: Clear move indicators after move execution
                    boardPanel.clearAllMoveIndicators();
                    chessBoard.setChosenTile(null);
                    return;
                }
                ...
                if (move != null) {
                    // MOVE PREVIEW SYSTEM: Clear move indicators before executing move
                    boardPanel.clearAllMoveIndicators();
                ...
                if (destinationTile.hasPiece()) {
                    if (chessBoard.getCurrentPlayer().getTeam() == destinationTile.getPiece().getTeam()) {
                        // MOVE PREVIEW SYSTEM: Clear old indicators and show new ones
                        boardPanel.clearAllMoveIndicators();
                        chessBoard.setChosenTile(destinationTile);
                        boardPanel.showLegalMovesForPiece(destinationTile);
                    }
                } else {
                    // MOVE PREVIEW SYSTEM: Clear move indicators when clicking empty square
                    boardPanel.clearAllMoveIndicators();
                    chessBoard.setChosenTile(null);
                }
            }
    ...
    private void sendMoveToServer(Move move, Board chessBoard) {
        ...
        if (client.socket == null || client.socket.isClosed()) {
            ...
            // Check for game ending conditions in offline mode
            if (GameLogic.checkGameEnd(chessBoard, boardPanel.getTable(), false)) {
                chessBoard.setChosenTile(null);
                return;
            }
    ...
    // ✅ MODIFIED: Dynamically scale and assign image to match tile size with proper coordinate handling
    public void assignTilePieceIcon(Board board) {
        // Get the actual board coordinate for this tile panel
        // The coordinate stored in this.coordinate should always be the actual board coordinate
        Tile thisTile = board.getTile(this.coordinate);
        if (thisTile == null) {
            System.out.println("DEBUG: Tile is null for coordinate " + this.coordinate);
            return;
        }
        
        // Tile piece assignment
    ...
    /**
     * MOVE PREVIEW SYSTEM: Override paintComponent to draw move indicators
     */
    @Override
    protected void paintComponent(Graphics g) {
        super.paintComponent(g);
        
        // Draw move indicator circle if this tile is a legal move destination
        if (showMoveIndicator) {
            Graphics2D g2d = (Graphics2D) g.create();
            g2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
            
            // Set circle color based on move safety
            Color circleColor = isSafeMove ? new Color(0, 255, 0, 180) : new Color(255, 0, 0, 180); // Green or Red with transparency
            g2d.setColor(circleColor);
            
            // Calculate circle position (center of tile)
            int tileWidth = getWidth();
            int tileHeight = getHeight();
            int circleX = (tileWidth - MOVE_INDICATOR_DIAMETER) / 2;
            int circleY = (tileHeight - MOVE_INDICATOR_DIAMETER) / 2;
            
            // Draw filled circle
            g2d.fillOval(circleX, circleY, MOVE_INDICATOR_DIAMETER, MOVE_INDICATOR_DIAMETER);
            
            // Draw circle border for better visibility
            g2d.setColor(circleColor.darker());
            g2d.drawOval(circleX, circleY, MOVE_INDICATOR_DIAMETER, MOVE_INDICATOR_DIAMETER);
    ...
    /**
     * MOVE PREVIEW SYSTEM: Show move indicator on this tile
     * @param isSafe true if the move is safe (green circle), false if dangerous (red circle)
     */
    public void showMoveIndicator(boolean isSafe) {
        this.showMoveIndicator = true;
        this.isSafeMove = isSafe;
        repaint();
    }
    
    /**
     * MOVE PREVIEW SYSTEM: Hide move indicator on this tile
     */
    public void hideMoveIndicator() {
        this.showMoveIndicator = false;
        repaint();
    }
